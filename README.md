...
Android control proof of concept demonstrated on one device. The structured-UI companion was built and exercised on a Nothing A059P running Android 16: Hebrew, English and mixed text, button click, scrolling, stale element handling and window-change handling passed on its test activity. The existing Shizuku control tool also passed a post-install status check. Manual service disable/re-enable, lock/unlock recovery, normal reboot recovery and the safe UI suite with USB disconnected passed. Temporary screen-awake retention, expiry and manual-lock precedence also passed. After reboot the companion reconnects, while non-root Shizuku still needs a manual Wireless debugging start. Unattended recovery and broader device compatibility remain unverified; see the detailed test status.
